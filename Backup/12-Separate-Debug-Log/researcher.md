...
- If `debug_config = on`, **append** to the run's log file:
  ```
  {timestamp} [DEBUG:config] agent=researcher resolved: provider=copilot model=Sonnet4 temp=0.2 max_tokens=2048 timeout=30
  ```
- Debug lines are written to the log file only — never included in the findings output
