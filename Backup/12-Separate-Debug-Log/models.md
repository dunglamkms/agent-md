...
> When any debug flag is `on`, the orchestrator creates the log file **before dispatch** and all agents **append lines in real time** as events occur.
> No debug content is written to the output artifact — the `.md` stays clean.
> Memory Agent **never reads `.log` files** — zero context window waste.
