...
- If **any** flag is `on`, create log file: `output/debug-{topic-slug}-{YYYY-MM-DD}.log`
- All `[DEBUG:*]` lines are **appended to the log file in real time** — never written to the output artifact
- If `debug_config = on`, write to log before dispatch:
  ```
  {timestamp} [DEBUG:config] agent=dryan resolved: provider=copilot model=Opus4.6 temp=0.3 max_tokens=1024 timeout=60
  ```
- If `debug_dispatch = on`, write to log each subagent dispatch and completion with resolved model info:
  ```
  {timestamp} [DEBUG:dispatch] sending topic="{topic}" to=researcher(Sonnet4,temp=0.2),writer(Sonnet4,temp=0.7),memory(Sonnet4,temp=0.1)
  {timestamp} [DEBUG:dispatch] researcher(Sonnet4,temp=0.2) completed in {N}s
  {timestamp} [DEBUG:dispatch] writer(Sonnet4,temp=0.7) completed in {N}s
  {timestamp} [DEBUG:dispatch] memory(Sonnet4,temp=0.1) completed in {N}s
  ```
- If `debug_merge = on`, write to log the merge inputs summary:
  ```
  {timestamp} [DEBUG:merge] inputs: research={N}chars draft={N}chars prior={N}chars sources={N}items
  {timestamp} [DEBUG:merge] output saved to {path}
  ```
- If all debug flags are `off`, no log file is created
