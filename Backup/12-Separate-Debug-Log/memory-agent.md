...
- If `debug_config = on`, **append** to the run's log file:
  ```
  {timestamp} [DEBUG:config] agent=memory resolved: provider=copilot model=Sonnet4 temp=0.1 max_tokens=1024 timeout=30
  ```
- If `debug_memory = on`, **append** to the run's log file:
  ```
  {timestamp} [DEBUG:memory] phase=read topic="{topic}" prior_found=true|false
  {timestamp} [DEBUG:memory] phase=write row="| {date} | {topic} | {path} |"
  ```
- Debug lines are written to the log file only — never included in agent output
- **Never read `.log` files** when loading prior context — only read `.md` artifacts
